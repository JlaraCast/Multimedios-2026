...
const API = {
    ...
    ENDPOINTS: {
        consultar: 'ListaEstudiantes.php',
        insertar: 'InsertarEstudiantes.php',
        actualizar: 'ActualizarEstudiantes.php',
        eliminar: 'BorrarEstudiantes.php'
    }
};
...
const elementos = {
    ...
    titulo: () => document.getElementById('tituloEstudiante'),
    btnSubmit: () => document.getElementById('btnSubmit'),
    alerta: () => document.getElementById('alertaFormulario'),
    tabla: () => document.getElementById('tablaEstudiante'),
    estudianteId: () => document.getElementById('estudianteId'),
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    elementos.form().addEventListener('submit', (e) => {
        ...
        if (validarFormulario()) {
            modoEdicion ? guardarEdicion() : insertarEstudiante();
        }
    });
    ...
    elementos.tabla().addEventListener('click', (e) => {
        if (e.target.classList.contains('btnEditar')) {
            ...
            eliminarEstudiante(e.target.dataset.id);
        }
    });
});
...
function obtenerDatos() {
    return {
        id: elementos.estudianteId().value || Math.floor(Math.random() * 10000),
        nombre: elementos.nombre().value.trim(),
        apellidopaterno: elementos.apellidopaterno().value.trim(),
...
function limpiarFormulario() {
    ...
    elementos.estudianteId().value = '';
}

function cambiarModoFormulario(editar = false) {
    modoEdicion = editar;
    elementos.titulo().textContent = editar ? 'Editar Estudiante' : 'Agregar Nuevo Estudiante';
    elementos.btnSubmit().textContent = editar ? 'Guardar Cambios' : 'Agregar Estudiante';
}
...
async function consultarAPI() {
    try {
        ...
        mostrarAlerta('Error al cargar los estudiantes', 'danger');
    }
}
...
function dibujarTabla(datos) {
    ...
    if (!datos.length) {
        tabla.innerHTML = '<tr><td colspan="7" class="text-center text-muted py-3">No hay estudiantes registrados aún</td></tr>';
        return;
    }
...
async function insertarEstudiante() {
    ...
    try {
        ...
        mostrarAlerta('Error al agregar el estudiante', 'danger');
    }
}
...
async function guardarEdicion() {
    ...
    try {
        ...
        mostrarAlerta('Error al actualizar el estudiante', 'danger');
    }
}
...
async function eliminarEstudiante(id) {
    ...
    try {
        ...
        mostrarAlerta('Error al eliminar el estudiante', 'danger');
    }
}

function editarEnFormulario(datos) {
    cambiarModoFormulario(true);
    elementos.estudianteId().value = datos.id;
